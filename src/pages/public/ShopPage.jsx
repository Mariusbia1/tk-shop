...
export default function ShopPage({ categorySlug }) {
  ...
  return <><SEO title="Collection crochet | TK SHOP" /><div className="bg-gradient-to-br from-mist via-linen to-champagne px-5 py-20 text-center dark:from-plum dark:via-[#3b2e14] dark:to-[#20190d]"><p className="text-xs font-bold uppercase tracking-[.2em] text-gold">{copy.eyebrow||'Notre crochet'}</p><h1 className="mt-4 font-display text-5xl">{copy.title||'La collection'}</h1><p className="mx-auto mt-4 max-w-xl text-sm leading-7 text-black/60">{copy.description||'Découvrez des créations crochetées lentement à la main, pensées pour être portées longtemps.'}</p></div>
