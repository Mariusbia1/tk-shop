...
export default function ShopPage({ categorySlug }) {
  const { products, categories, content } = useCatalog()
  const copy = content.collection || {}
  ...
  const pageTitle = currentCat
    ? `${currentCat.name} en crochet | TK SHOP`
    : 'Collection crochet fait main | Robes, Ensembles & Sacs | TK SHOP'
  const pageDesc = currentCat
    ? `Explorez notre sélection de ${currentCat.name.toLowerCase()} crochetées à la main par TK SHOP. Des pièces artisanales uniques et sur-mesure confectionnées avec passion.`
    : (copy.description || 'Découvrez notre collection de créations crochetées lentement à la main : robes, ensembles, tops, jupes et accessoires uniques.')

  const breadcrumbs = [
    { name: 'Accueil', url: '/' },
    { name: 'Collection', url: '/collections' },
    ...(currentCat ? [{ name: currentCat.name, url: `/categories/${currentCat.slug}` }] : []),
  ]
  ...
  return <>
    <SEO
      ...
      keywords={[
        'collection crochet',
        'crochet tk shop',
        'pièces crochet artisanales',
        currentCat ? currentCat.name.toLowerCase() : 'robes ensembles sacs',
        'artisanat crochet',
      ]}
      path={currentCat ? `/categories/${currentCat.slug}` : '/collections'}
      breadcrumbs={breadcrumbs}
    />
    <div className="bg-gradient-to-br from-mist via-linen to-champagne px-5 py-20 text-center dark:from-plum dark:via-[#3b2e14] dark:to-[#20190d]"><p className="text-xs font-bold uppercase tracking-[.2em] text-gold">{copy.eyebrow||'Notre crochet'}</p><h1 className="mt-4 font-display text-5xl">{copy.title||'La collection'}</h1><p className="mx-auto mt-4 max-w-xl text-sm leading-7 text-black/60">{copy.description||'Découvrez des créations crochetées lentement à la main, pensées pour être portées longtemps.'}</p></div>
