...
export default function ProductPage() {
  ...
  const validate = () => {
    ...
    if (product.sizes.length && !sizes.length) { toast.error('Veuillez choisir au moins une taille.'); return false }
    if (product.colors.length && !colors.length) { toast.error('Veuillez choisir au moins une couleur.'); return false }
    return true
  }
  const toggleOption = (value, selected, setSelected) => setSelected(selected.includes(value) ? selected.filter(item => item !== value) : [...selected, value])
  const selectedOptions = { size: sizes.join(', '), color: colors.join(', '), quantity, measurements, note }
  const add = () => validate() && addItem(product, selectedOptions)
  const orderNow = () => {
    if (!validate()) return
  ...
  return <><SEO title={`${product.name} | TK SHOP`} /><div className="mx-auto max-w-7xl px-5 py-8 lg:px-8"><nav className="mb-8 text-xs text-black/50"><Link to="/">Accueil</Link> / <Link to="/collections">Collection</Link> / {product.name}</nav>
    ...
    <div className="lg:py-6"><p className="text-xs font-bold uppercase tracking-[.2em] text-gold">{product.category}</p><h1 className="mt-3 font-display text-4xl md:text-5xl">{product.name}</h1><div className="mt-4 flex flex-wrap items-center gap-3"><p className="text-xl font-semibold">{formatCurrency(product.price)}</p><span className="rounded-full bg-mist px-3 py-1.5 text-[10px] font-bold uppercase tracking-wider text-gold">{product.stockStatus === 'Disponible' ? 'Disponible immédiatement' : product.stockStatus}</span></div><p className="mt-6 leading-8 text-black/60">{product.description}</p>
      {product.colors.length>0&&<fieldset className="mt-8"><legend className="mb-3 text-sm font-bold">Couleurs <span className="font-normal text-black/45">— plusieurs choix possibles</span></legend><div className="flex flex-wrap gap-2">{product.colors.map(c=><button type="button" aria-pressed={colors.includes(c)} key={c} onClick={()=>toggleOption(c,colors,setColors)} className={`border px-4 py-2 text-sm ${colors.includes(c)?'border-ink bg-ink text-white':'border-black/20'}`}>{c}</button>)}</div>{colors.length>0&&<p className="mt-2 text-xs text-gold">{colors.join(' · ')}</p>}</fieldset>}
      {product.sizes.length>0&&<fieldset className="mt-7"><legend className="mb-3 text-sm font-bold">Tailles <span className="font-normal text-black/45">— plusieurs choix possibles</span></legend><div className="flex flex-wrap gap-2">{product.sizes.map(s=><button type="button" aria-pressed={sizes.includes(s)} key={s} onClick={()=>toggleOption(s,sizes,setSizes)} className={`grid h-11 min-w-11 place-items-center border px-3 text-sm ${sizes.includes(s)?'border-ink bg-ink text-white':'border-black/20'}`}>{s}</button>)}</div>{sizes.length>0&&<p className="mt-2 text-xs text-gold">{sizes.join(' · ')}</p>}</fieldset>}
