...
const categoryShapes = [
  'rounded-[2.75rem_1.25rem_2.75rem_1.25rem]',
  'rounded-[1.25rem_2.75rem_1.25rem_2.75rem]',
  'rounded-[2.75rem_2.75rem_1.25rem_1.25rem]',
  'rounded-[1.25rem_1.25rem_2.75rem_2.75rem]',
]
...
export default function HomePage() {
  ...
  return <><SEO title="TK SHOP | Créations crochetées à la main" />
    ...
    <section className="mx-auto max-w-7xl px-5 py-24 lg:px-8"><SectionTitle eyebrow="Explorer">À chacune son allure</SectionTitle><div className="grid grid-cols-2 items-start gap-3 md:grid-cols-3 md:gap-6">{categories.map((c, i) => {const shape=categoryShapes[i%categoryShapes.length];return <Link key={c.slug} to={`/categories/${c.slug}`} className={`${shape} group relative isolate aspect-[4/5] self-start overflow-hidden border border-gold/15 bg-mist shadow-[0_18px_45px_rgba(74,56,20,.10)]`}>{c.image?<img src={c.image} alt={c.name} loading="lazy" className="h-full w-full object-cover object-center transition duration-700 ease-out group-hover:scale-105" />:<div className="h-full w-full bg-gradient-to-br from-linen via-mist to-sand"/>}<div className="absolute inset-0 bg-gradient-to-t from-plum/85 via-transparent to-transparent" /><div className="absolute inset-3 rounded-[inherit] border border-white/25 opacity-60 transition group-hover:inset-4"/><span className="absolute inset-x-0 bottom-0 p-5 font-display text-xl text-white drop-shadow-lg md:p-7 md:text-2xl">{c.name}<span className="mt-2 block h-px w-10 bg-sand transition-all duration-500 group-hover:w-20"/></span></Link>})}</div></section>
