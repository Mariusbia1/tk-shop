...
export default function HomePage() {
  ...
  return <><SEO title="TK SHOP | Créations crochetées à la main" />
    ...
    <section className="rounded-[3rem] bg-mist py-20 md:mx-5"><div className="mx-auto max-w-7xl px-5 lg:px-8"><SectionTitle eyebrow="Crochets coup de cœur">Nos créations signatures</SectionTitle><div className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-4 md:gap-7">{products.filter(p => p.featured).map(p => <ProductCard key={p.id} product={p} />)}</div><div className="mt-12 text-center"><Button to="/collections" variant="outline">Voir toute la collection <ArrowRight className="h-4 w-4" /></Button></div></div></section>
    <section className="mx-auto max-w-7xl px-5 py-24 lg:px-8"><SectionTitle eyebrow="Carnet d’atelier">Nos créations prennent vie</SectionTitle><p className="mx-auto -mt-5 mb-10 max-w-2xl text-center text-sm leading-7 text-black/60">Découvrez l’allure et les détails des créations TK SHOP à travers une sélection de photos et de vidéos.</p><div className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3">{gallery.slice(0,6).map((item,index)=><Link key={item.id} to="/galerie" className="group relative overflow-hidden rounded-[2rem] bg-mist shadow-soft">{item.mediaType==='video'?<video src={item.image} muted autoPlay loop playsInline preload="metadata" className="aspect-[4/5] h-full w-full object-cover"/>:<img src={item.image} alt={item.title} loading="lazy" className="aspect-[4/5] h-full w-full object-cover transition duration-700 group-hover:scale-105"/>}<span className="absolute inset-0 bg-gradient-to-t from-black/75 via-transparent to-transparent"/><span className="absolute inset-x-0 bottom-0 p-6 font-display text-2xl text-white">{item.title}<span className="mt-2 block text-[10px] font-sans font-bold uppercase tracking-[.18em] text-sand">{item.mediaType==='video'?'Voir la vidéo':'Voir la galerie'}</span></span></Link>)}</div><div className="mt-10 text-center"><Button to="/galerie" variant="outline">Découvrir toute la galerie <ArrowRight className="h-4 w-4"/></Button></div></section>
