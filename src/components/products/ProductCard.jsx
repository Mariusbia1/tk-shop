...
export default function ProductCard({ product }) {
  ...
  const canQuickAdd = !product.sizes.length && !product.colors.length
  return <article className="group rounded-[1.75rem] bg-white p-2 shadow-soft transition duration-500 hover:-translate-y-1 dark:bg-white/5">
    <div className="relative aspect-[3/4] overflow-hidden rounded-[1.35rem] bg-mist"><Link to={`/collections/${product.slug}`}><img loading="lazy" src={product.images[0]} alt={product.name} className="h-full w-full object-cover transition duration-700 group-hover:scale-105" /></Link>
      <span className="absolute left-3 top-3 rounded-full bg-ivory/90 px-3 py-1 text-[10px] font-bold uppercase tracking-wider text-gold backdrop-blur">{product.newProduct ? 'Nouveau' : product.popular ? 'Populaire' : product.stockStatus}</span>
