...
export default function Footer() {
  ...
  return <footer className="bg-gradient-to-br from-[#413416] via-plum to-ink text-white"><div className="mx-auto grid max-w-7xl gap-10 px-5 py-16 md:grid-cols-4 lg:px-8">
    ...
    <div><h3 className="text-xs font-bold uppercase tracking-[.18em] text-sand">Nous contacter</h3><div className="mt-5 grid gap-3 text-sm text-white/70"><span>{settings.phone}</span><span>{settings.email}</span><span>{settings.address}</span><div className="flex gap-4">{settings.instagram&&<a href={settings.instagram} target="_blank" rel="noreferrer" aria-label="Instagram"><Instagram /></a>}{settings.facebook&&<a href={settings.facebook} target="_blank" rel="noreferrer" aria-label="Facebook"><Facebook /></a>}</div></div></div>
  </div><div className="border-t border-white/10 px-5 py-5 text-center text-xs text-white/45">© 2026 TK SHOP. Tous droits réservés.</div></footer>
}
