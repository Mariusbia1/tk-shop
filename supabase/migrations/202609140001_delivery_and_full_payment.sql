-- Migration : Gestion de la livraison réglée à la réception & Choix d'acompte 50% ou 100% total
-- 1. Mise à jour de create_secure_order pour ne plus ajouter de frais de livraison fixes au total
...
begin
  ...
  -- Les frais de livraison ne sont pas facturés en ligne : ils sont réglés directement à la livraison
  insert into public.orders (
    ...
    calculated_subtotal, 0, calculated_subtotal, true
...
-- 2. Mise à jour de create_order_with_payment pour gérer acompte (50%) ou paiement intégral (100%)
...
declare
  ...
  method text;
  pay_type text;
  deposit_calc integer;
  remain_calc integer;
begin
  result := public.create_secure_order(payload);
  order_id := (result ->> 'id')::uuid;
  order_total := (result ->> 'total')::integer;
  method := case when payload #>> '{customer,payment_method}' in ('fedapay', 'online') then 'fedapay' else 'whatsapp' end;
  pay_type := coalesce(payload #>> '{customer,payment_type}', 'deposit');

  if method = 'fedapay' then
    if pay_type = 'full' then
      deposit_calc := order_total;
      remain_calc := 0;
    else
      deposit_calc := ceil(order_total / 2.0)::integer;
      remain_calc := floor(order_total / 2.0)::integer;
    end if;
  else
    deposit_calc := 0;
    remain_calc := order_total;
  end if;

  update public.orders
  set payment_method = method,
      payment_status = case when method = 'fedapay' then 'pending' else 'unpaid' end,
      deposit_amount = deposit_calc,
      remaining_amount = remain_calc,
      delivery_fee = 0,
      total = subtotal,
